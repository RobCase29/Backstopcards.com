...
const queue = db
  .prepare(`
    SELECT
      queue.player_name AS playerName,
      queue.release_year AS releaseYear,
      queue.priority,
      COALESCE((
        SELECT MAX(releases.release_name)
        FROM checklist_cards cards
        JOIN checklist_releases releases ON releases.release_key = cards.release_key
        WHERE cards.release_year = queue.release_year
          AND lower(cards.player_name) = lower(queue.player_name)
      ), queue.release_year || ' Bowman') AS releaseName
    FROM canonical_refresh_queue queue
    WHERE queue.player_name IS NOT NULL AND trim(queue.player_name) <> '' AND queue.release_year IS NOT NULL
    ORDER BY queue.priority DESC, queue.release_year DESC, queue.player_name
  `)
  .all()
...
const queueTuples = queue.map((row) => [compact(row.playerName), Number(row.releaseYear), Number(row.priority) || 0, compact(row.releaseName)])
